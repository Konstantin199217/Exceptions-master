public class ExceptionExample {
    ...
    /*В этом примере у нас есть два статических метода: divideNumbers и performCalculation.
        Метод divideNumbers(a, b) выполняет деление двух чисел a и b.
        Он перехватывает исключение ArithmeticException, которое возникает при делении на ноль,
        и обрабатывает его на текущем уровне абстракции путем вывода сообщения об ошибке.
        Метод performCalculation(x, y) вызывает метод divideNumbers для выполнения деления чисел x и y.
        Если результат не является NaN (не число), то он обрабатывается на этом же уровне, например,
        выводится на экран. Если возникает любое другое исключение,
        оно перехватывается на более высоком уровне в блоке catch (Exception e),
        где выводится соответствующее сообщение об ошибке.
        Таким образом, этот пример также следует принципу одного уровня абстракции,
        где исключения обрабатываются на том уровне, где есть достаточно информации для их корректной обработки.
     */
